...
int main(int argc, char** argv)
{
    ...
    state_type s_eta={{20.0/4e3,20.0/4e3}};
    /* lower bounds of the hyper-rectangle */
    state_type lb={{0.649,4.949}};
    /* upper bounds of the hyper-rectangle */
    state_type ub={{1.65,5.95}};
    scots::SymbolicSet ss_pre(manager,state_dim,lb,ub,s_eta);
    scots::SymbolicSet ss_post(manager,state_dim,lb,ub,s_eta);
    std::cout << "\n state alphabet details: \n";
    ss_pre.print_info();

    /* construct grid for the input alphabet */
    /* hyper-rectangle [1,2] with grid node distance 1 */
    auto i_eta = input_type{{1.0 / divider}}; //Original value 1
    scots::SymbolicSet ss_input(manager,input_dim,input_type{{.99}},input_type{{2.1}},i_eta);
    ...
    /* lambda for target 1*/
    auto target_1 = [&ss_pre,&s_eta](const scots::abs_type& idx) {
    ...
    /* lambda for target 2*/
    auto target_2 = [&ss_pre,&s_eta](const scots::abs_type& idx) {
    ...
    for(i = 0; i < 2; i++)
    {
        std::cout << "Controller size before re-ordering: " << controller[i].nodeCount() << std::endl;
        
        string file_name = string("dcdc_rec_") + std::to_string(i+1) + suffix;
        
        std::cout << "write " << file_name << ".scs to file : ";
